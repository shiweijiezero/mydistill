...
class PLDataModule(LightningDataModule):
    def __init__(self,
        ...
        logger.info(f"Loading text from xsum")
    ...
    def test_collate_fn(self, batch):
        ...
        # summary = item["summary"]
        # text = document + "\n summary:"
        # tk_t = self.tokenizer(text, return_tensors="pt", truncation=True, max_length=1000)
        # # {'input_ids': tensor([[8888,  318]]), 'attention_mask': tensor([[1, 1]])}
        # input_ids = tk_t["input_ids"]
        # attention_mask = tk_t["attention_mask"]
        ...
        for item in batch:
            ...
            summary = item["summary"]
            text = document + "\n summary:"
            tk_t = self.tokenizer(text, return_tensors="pt", truncation=True, max_length=1024)
            # {'input_ids': tensor([[8888,  318]]), 'attention_mask': tensor([[1, 1]])}
            res.append(tk_t)
            input_ids.append(tk_t["input_ids"])
            attention_masks.append(tk_t["attention_mask"])
            documents.append(document)
            summaries.append(summary)

        return input_ids,attention_masks,documents,summaries
