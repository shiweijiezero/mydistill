...
class Wrapper(pl.LightningModule):
    ...
    def training_step(self, input_, batch_idx):
        ...
        log_dic = {
            ...
            'loss/last_loss_cos': self.last_loss_cos,
            "loss/lr": lr,
        }
        # if(batch_idx%5==0):
        #     print(f"self.lr:{lr}")
        ...
        log_dic["usage/memory_usage"] = psutil.virtual_memory()._asdict()["used"] / 1_000_000
    ...
    def test_step(self, input_, batch_idx):
        input_ids, attention_masks, documents, summaries = input_
        self.outs = []
        self.labels = []
        self.t_outs = []

        input_ids = [input_ids[i].to(self.device) for i in range(len(input_ids))]
        attention_masks = [attention_masks[i].to(self.device) for i in range(len(attention_masks))]
        ...
        for i in range(len(summaries)):
            ...
            teacher_out = self.teacher.generate(
                ...
                max_new_tokens=1024 - input_ids[i].shape[1] - 1,
                do_sample=True,
                # top_k=5,
                # top_p=0.85,
                # temperature=0.8,
            ...
            sample_outputs = self.student.generate(
                input_ids=input_ids[i],
                attention_mask=attention_masks[i],
                max_new_tokens=1024 - input_ids[i].shape[1] - 1,
                do_sample=True,
            ...
            output = self.tokenizer.decode(sample_outputs[0], skip_special_tokens=True)
            output = output.split("summary:")[-1]
            t_o = self.tokenizer.decode(teacher_out[0], skip_special_tokens=True)
            t_o = t_o.split("summary:")[-1]
            self.outs.append(output)
            self.t_outs.append(t_o)
            self.labels.append(label)

    def on_test_epoch_end(self, *dic_lst) -> None:
        logger.info(f"--- on_test_epoch_end")
        logger.info(f"out:{len(self.outs)}, {self.outs[0]}")
        logger.info(f"t_outs:{len(self.t_outs)}, {self.t_outs[0]}")
        logger.info(f"labels:{len(self.labels)}, {self.labels[0]}")

        rouge = evaluate.load('rouge')
        results = rouge.compute(predictions=self.outs,
                                references=self.labels,
                                use_aggregator=True
                                )
        t_results = rouge.compute(predictions=self.t_outs,
                                  references=self.labels,
                                  use_aggregator=True
                                  )
    ...
    def configure_optimizers(self):
        ...
        self.scheduler = {"scheduler": self.scheduler, "interval": "step", "frequency": 1}
